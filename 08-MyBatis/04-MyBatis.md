...
1. session.commit() 会清空缓存

2. sqlSession 执行 insert、update、delete 等操作 commit 提交后会清空缓存区域

3. session.close() 会清空缓存
...
​		查询的总次数作为分母，二级缓存使用的次数作为分子，分子/分母的结果就是命中率

##### 4.1.3、一二级缓存同时在启用时的步骤

​		先从二级缓存查找，查找到了直接拿出返回

​		如果二级缓存查找不到，到一级缓存中,拿到了直接返回

​		如果一级缓存查找不到，发 sql 语句到数据库查找,将查询结果放到一级缓存，session 关闭会清空一级缓存，会将数据放到二级缓存中

​		session 进行 insert、update、delete 后提交会清空一二级缓存
...
​		FIFO – First In First Out，按照先进先出的原则移除

​		SOFT – Soft Reference，基于垃圾回收器的软引用原则

​		WEAK – Weak Reference，基于垃圾回收器的弱引用原则
